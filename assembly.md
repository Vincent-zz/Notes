...
主板（CPU和内存）控制接口卡/IO接口（如显卡、网卡）控制外部设备（如显示器） 

现在很多电脑将CPU、内存、IO接口卡都集成在一起，构成嵌入式计算机（单片机） 
...
- 数据总线（data bus）：传送数据，越宽速度越快，每根线都是双向的
